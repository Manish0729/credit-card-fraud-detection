#!/usr/bin/env python3
"""
Demo script for Credit Card Fraud Detection project with REAL dataset.
This script is adapted for the fraudTrain.csv dataset structure.
"""
...
def load_real_data():
    """Load the real fraud dataset."""
    print("📥 Loading Real Fraud Dataset...")
...
def save_models(trained_models, scaler, label_encoders):
    ...
    for name, model in trained_models.items():
        filename = f'models/{name.lower().replace(" ", "_")}_real.pkl'
        joblib.dump(model, filename)
        print(f"✅ {name} saved to {filename}")
    
    # Save scaler
    scaler_filename = 'models/scaler_real.pkl'
    joblib.dump(scaler, scaler_filename)
    print(f"✅ Scaler saved to {scaler_filename}")
    
    # Save label encoders
    if label_encoders:
        encoders_filename = 'models/label_encoders_real.pkl'
...
def main():
    """Main function to run the complete fraud detection pipeline with real data."""
    print("🚀 Credit Card Fraud Detection - REAL DATA Pipeline")
    print("=" * 70)
